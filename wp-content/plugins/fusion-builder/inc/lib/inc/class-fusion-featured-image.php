...
class Fusion_Featured_Image {
	...
	public function __construct( $args ) {
		...
		$this->defaults = [
			'id'           => 'featured-image-2',
			'post_type'    => 'page',
			'name'         => esc_html__( 'Featured Image 2', 'fusion-builder' ),
			'label_set'    => esc_html__( 'Set featured image 2', 'fusion-builder' ),
			'label_remove' => esc_html__( 'Remove featured image 2', 'fusion-builder' ),
		];
	...
	public function meta_box_info_content() {
		...
		echo sprintf( esc_html__( 'To control the amount of featured image boxes, visit %s.', 'fusion-builder' ), '<a href="' . esc_url_raw( admin_url( 'themes.php?page=avada_options#posts_slideshow_number' ) ) . '" target="_blank" rel="noopener noreferrer">' . esc_html__( 'Fusion Theme Options', 'fusion-builder' ) . '</a>' );
	}
