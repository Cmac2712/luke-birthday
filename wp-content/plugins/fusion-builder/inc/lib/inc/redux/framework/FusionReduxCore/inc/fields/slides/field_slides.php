	...
	class FusionReduxFramework_slides {
		...
		public function render () {
			...
			$defaults = array(
				...
				'content_title' => __ ( 'Slide', 'fusion-builder' )
			);

			$this->field = wp_parse_args ( $this->field, $defaults );

			echo '<div class="fusionredux-slides-accordion" data-new-content-title="' . esc_attr ( sprintf ( __ ( 'New %s', 'fusion-builder' ), $this->field[ 'content_title' ] ) ) . '">';
			...
			if ( isset ( $this->value ) && is_array ( $this->value ) && !empty ( $this->value ) ) {
				...
				foreach ( $slides as $slide ) {
					...
					echo '<span class="button media_upload_button" id="add_' . $x . '">' . __ ( 'Upload', 'fusion-builder' ) . '</span>';
					...
					echo '<span class="button remove-image' . $hide . '" id="reset_' . $x . '" rel="' . $slide[ 'attachment_id' ] . '">' . __ ( 'Remove', 'fusion-builder' ) . '</span>';
					...
					$placeholder = ( isset ( $this->field[ 'placeholder' ][ 'title' ] ) ) ? esc_attr ( $this->field[ 'placeholder' ][ 'title' ] ) : __ ( 'Title', 'fusion-builder' );
					...
					if ( $this->field[ 'show' ][ 'description' ] ) {
						$placeholder = ( isset ( $this->field[ 'placeholder' ][ 'description' ] ) ) ? esc_attr ( $this->field[ 'placeholder' ][ 'description' ] ) : __ ( 'Description', 'fusion-builder' );
					...
					$placeholder = ( isset ( $this->field[ 'placeholder' ][ 'url' ] ) ) ? esc_attr ( $this->field[ 'placeholder' ][ 'url' ] ) : __ ( 'URL', 'fusion-builder' );
					...
					echo '<li><a href="javascript:void(0);" class="button deletion fusionredux-slides-remove">' . __ ( 'Delete', 'fusion-builder' ) . '</a></li>';
			...
			if ( $x == 0 ) {
				echo '<div class="fusionredux-slides-accordion-group"><fieldset class="fusionredux-field" data-id="' . $this->field[ 'id' ] . '"><h3><span class="fusionredux-slides-header">' . esc_attr ( sprintf ( __ ( 'New %s', 'fusion-builder' ), $this->field[ 'content_title' ] ) ) . '</span></h3><div>';
				...
				echo '<span class="button remove-image' . $hide . '" id="reset_' . $x . '" rel="' . $this->parent->args[ 'opt_name' ] . '[' . $this->field[ 'id' ] . '][attachment_id]">' . __ ( 'Remove', 'fusion-builder' ) . '</span>';
			...
			echo '</div><a href="javascript:void(0);" class="button fusionredux-slides-add button-primary" rel-id="' . $this->field[ 'id' ] . '-ul" rel-name="' . $this->field[ 'name' ] . '[title][]' . $this->field['name_suffix'] .'">' . sprintf ( __ ( 'Add %s', 'fusion-builder' ), $this->field[ 'content_title' ] ) . '</a><br/>';
		}
